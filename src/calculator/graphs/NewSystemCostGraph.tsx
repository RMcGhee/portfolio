...
const NewSystemCostGraph: React.FC<NewSystemCostGraphProps> = ({
  ...
  const monthlyOldHvacCost = months.map((month) => {
    ...
    if (cdd > 0) {
      monthCost +=
        ((cdd * formData.averagekBTUdd) / acCop / btuInkWh) *
        1000 *
        1.1 *
        electricPrice;
    }
    if (hdd > 0) {
      monthCost +=
        ((hdd * formData.averagekBTUdd) / furnaceEfficiency / btuInCcf) *
        1000 *
        0.85 *
        gasPrice;
    }
  ...
  const monthlyOldTotalCost = monthlyOldHvacCost.map((cost) => {
    return (
      cost +
      formData.baseElectricUsage * electricPrice +
      formData.baseGasUsage * gasPrice
    );
  });

  const desiredHvacYearlyCost = monthlyHVACCost.reduce(
    (acc, next) => acc + next,
  );
  const desiredTotalYearlyCost = monthlyTotalCost.reduce(
    (acc, next) => acc + next,
  );
  const oldHvacYearlyCost = monthlyOldHvacCost.reduce(
    (acc, next) => acc + next,
  );
